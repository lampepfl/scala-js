...
final class IRLoader extends Analyzer.InputProvider with MethodSynthesizer.InputProvider {
  ...
  private def maybeGet[T](className: ClassName, f: ClassDefAndInfoCache.Update => T)(
    ...
    classNameToFile.get(className).map { irFile =>
      cache.getOrElseUpdate(className, new ClassDefAndInfoCache).update(irFile, logger).map(f)
    }
  }
...
private final class ClassDefAndInfoCache {
  ...
  def update(irFile: IRFileImpl, logger: Logger)(
    ...
    if (!cacheUsed) {
      ...
      if (version.isEmpty || newVersion.isEmpty ||
        ...
        cacheUpdate = irFile.tree.map { tree =>
          if (ClassDefChecker.check(tree, logger) != 0) {
            throw new Exception("IR Checking failed")
          }
